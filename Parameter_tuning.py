...
# crossover_rate = float(input('Please input the size of Crossover Rate: ') or 0.4)  # default value is 0.8
# mutation_rate = float(input('Please input the size of Mutation Rate: ') or 0.1)  # default value is 0.3
num_generations = int(input('Please input number of generations: ') or 100)  # default value is 1000
# max_episodes = int(input('Please input number of max iterations: ') or 2)  # default value is 1
...
def distance_calculation(data):
    distance_mat = [[] for i in range(5)]
    for i in range(5):
        for j in range(len(data[i])):
            distance_temp = []
            for k in range(len(data[i])):
                if j != k:
                    distance_temp.append(sum(abs(data[i].loc[j] - data[i].loc[k])))  # 记录点j和其他点的距离
            distance_mat[i].append(min(distance_temp))


if __name__ == "__main__":
    read_path = r'C:\Users\Lipei\Desktop' + '\\Program\\Data\\' + 'Orthogonal Array.xlsx'
    para_tmp = pd.read_excel(read_path, sheet_name=0, index_col=[0], header=0)
    pr = pd.DataFrame(para_tmp)
    Q.Job_num = 20
    Q.stage_num = 3
    machine_num = 3
    ...
    for i in range(5):
        name = 'SM_' + str(i + 1)
        pr[name] = 0  # 新增五列存储SM数值
    n_episodes = len(pr)  # 总共有27轮实验，每次实验算五次

    for cnt in range(n_episodes):  # 27个实例
        print('THIS IS', cnt, 'EPISODE')
        '''------------Read the parameter-------------'''
        population_size = pr.loc[cnt + 1, 'Psize']
        crossover_rate = pr.loc[cnt + 1, 'Pc']
        mutation_rate = pr.loc[cnt + 1, 'Pm']
        max_episodes = pr.loc[cnt + 1, 'Q-iter']
        CT = pr.loc[cnt + 1, 'CT']
        CPU_Time = CT * Q.Job_num * Q.stage_num  # 运行时间
        output_sum = []  # 每次

        for j_ in range(5):  # 五次实验
            print('This is', j_, 'experiment of', cnt, 'EPISODE')
            population_list = Q.Initialization(population_size, TOU_mat)  # 初始化个体
            best_list = []  # 用来记录最好的个体
            best_front = []  # 用来记录最好的个体编号
            ...
            output = pd.DataFrame(
                np.zeros((len(best_list) + 1, len(Name))),  # output
        ...
        path = r'C:\Users\Lipei\Desktop' + '\\Program\\Outcome_2023\\' + 'Parameter_tuning.xlsx'
        sv_path = pd.ExcelWriter(path, mode='a', engine='openpyxl', if_sheet_exists='new')
        output_sum.to_excel(sv_path, sheet_name=str(cnt + 1))
        sv_path.save()
        sv_path.close()
