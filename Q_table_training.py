...
max_episodes = int(input('Please input number of max iterations: ') or 5)  # default value is 1
    ...
    for Q.Job_num in JOB_LIST:
        for Q.stage_num in STAGE_LIST:
            for machine_num in MACHINE_LIST:
                ...
                Q_table_1 = Q.build_q_table(ACTIONS_1)
                Q_table_2 = Q.build_q_table(ACTIONS_2)
                ...
                time_start = time.time()  # 记录开始时间
                for episode in range(3):  # 每个算例训练3次
                    for n in range(num_generations):
                        ...
                        population_list, new_pop_idx = Q.elite_preservation(population_size, total_front, total_obj_record,
                        ...
                        Q_list = population_list[:5:]  # 取出五个变异，记录
                        Q_obj = [parent_obj_record[k] for k in range(5)]  # 取出目标函数
                        for _ in range(3):  # 记录每个目标函数的最大值和最小值
                            sort_temp = sorted(parent_obj_record.items(), key=lambda x: x[1][_])
                            obj_min[_], obj_max[_] = sort_temp[0][1][_], sort_temp[-1][1][_]

                        for _ in range(len(Q_list)):  # 对offspring_front里最好的个体进行局部搜索'''
                            improvement = 0  # 每一轮都初始化improvement
                            x = copy.deepcopy(Q_list[_])
                            x_obj_record = Q_obj[_]  # objectives of solution x
                            not_improvement = 0
                            ...
                            while not_improvement <= max_episodes:
                                ...
                                if x2_weighted_obj < x_weighted_obj:  # 解提升了
                                    ...
                                    reward_1 = 0
                                    for j in range(3):  # we have 3 objectives
                                        a_1 = obj_min[j] * (x_obj_record[j] - x2_obj_record[j]) + 1
                                        b_1 = x_obj_record[j] * (obj_max[j] - obj_min[j]) + 1
                                        reward_1 += a_1 / b_1
                                    x_obj_record = copy.deepcopy(x2_obj_record)  # x的目标值更新为x_

                                    Q_table_1.loc[state_1, action_1] = Q.learn(Q_table_1, state_1, action_1,
                                                                               reward_1, next_state_1)
                                    # Update the Q_table value
                                    print('DC improve')

                                else:
                                    not_improvement += 1
                                    if np.random.random() < EPISILON:  # print(_, 'not improved'),被不良取代
                                ...
                                while available_states:
                                    ...
                                    if type(x_[0]) is list:  # 如果解的个数大于1，则取出最好的解
                                        ...
                                        for i in range(len(searched_obj_record)):  # 对于每一个个体都计算加权目标
                                            temp = Q.weighted_obj(searched_obj_record[i], obj_min, obj_max)
                                    ...
                                    if x2_weighted_obj < x_weighted_obj:  # 解提升了
                                        ...
                                        Q_table_2.loc[state_2, action_2] = Q.learn(Q_table_2, state_2, action_2,
                                                                                   reward_2, next_state_2)
                            ...
                            Q_list[_] = copy.deepcopy(x)  # 更新Q_list里的个体
                            Q_obj[_] = copy.deepcopy(x_obj_record)

                        population_list[0:5] = Q_list  # 更新个体
                        for k in range(5):
                        ...
                        if n == 0:  # 和历史最优解进行比对，如果第一次循环，，则全局最优解就是当前的population——list
                            ...
                            now_best_front, now_best_rank = Q.non_dominated_sorting(population_size,
                                                                                    total_obj)  # 把局部最优和全局最优比对
                            best_list, best_pop_idx = Q.elite_preservation(population_size, now_best_front, total_obj,
                                                                           total_list)
